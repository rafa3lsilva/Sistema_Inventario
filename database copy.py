...
def create_user(username, hashed_password, role):
    ...
    Retorna True se criado com sucesso, False se o usuário já existe.
    """
    try:
        conn = get_db_connection()
        conn.execute(
...
def admin_exists():
    conn = get_db_connection()
    result = conn.execute("SELECT 1 FROM usuarios WHERE role = 'admin' LIMIT 1").fetchone()
    conn.close()
    return result is not None

def get_all_users():
    conn = get_db_connection()
    users = [row['username'] for row in conn.execute("SELECT username FROM usuarios").fetchall()]
    conn.close()
    return users
...
def add_product(ean, descricao):
    conn = get_db_connection()
    try:
        conn.execute("INSERT INTO produtos (ean, descricao) VALUES (?, ?)", (ean, descricao))
        conn.commit()
    except sqlite3.IntegrityError:
        pass
    finally:
...
def get_total_count(ean):
    conn = get_db_connection()
    result = conn.execute("SELECT SUM(quantidade) as total FROM contagens WHERE ean = ?", (ean,)).fetchone()
    conn.close()
...
def add_or_update_count(username, ean, quantidade):
    ...
    result = conn.execute("SELECT quantidade FROM contagens WHERE username = ? AND ean = ?", (username, ean)).fetchone()
    if result:
        nova_qtd = result["quantidade"] + quantidade
        conn.execute("UPDATE contagens SET quantidade = ?, timestamp = CURRENT_TIMESTAMP WHERE username = ? AND ean = ?", (nova_qtd, username, ean))
    else:
        conn.execute("INSERT INTO contagens (username, ean, quantidade) VALUES (?, ?, ?)", (username, ean, quantidade))
    conn.commit()
    conn.close()
