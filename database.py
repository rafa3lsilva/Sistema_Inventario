...
def check_login(username, password):
    ...
    user = conn.execute(
        text("SELECT username, role FROM usuarios WHERE username = :username AND password = :password"),
        {"username": username, "password": hashed_password}
    ).fetchone()
    conn.close()
    return {"username": user[0], "role": user[1]} if user else None
...
def get_all_users():
    with get_db_connection() as conn:
        rows = conn.execute(text("SELECT username FROM usuarios")).mappings().all()
...
def get_product_info(ean):
    conn = get_db_connection()
    produto = conn.execute(text("SELECT * FROM produtos WHERE ean = :ean"), {"ean": ean}).mappings().fetchone()
    conn.close()
    return {"ean": produto["ean"], "descricao": produto["descricao"]} if produto else None
        ...
        for _, row in df_csv.iterrows():
            ...
            if existente:
                if existente["descricao"] != descricao:
                    conn.execute(
                        text("UPDATE produtos SET descricao = :descricao WHERE ean = :ean"),
                        {"descricao": descricao, "ean": ean}
                    )
            else:
                conn.execute(
                    text("INSERT INTO produtos (ean, descricao) VALUES (:ean, :descricao)"),
                    {"ean": ean, "descricao": descricao}
                )
...
def add_or_update_count(username, ean, quantidade):
    with get_db_connection() as conn:
        result = conn.execute(
            text("SELECT quantidade FROM contagens WHERE username = :username AND ean = :ean"),
            {"username": username, "ean": ean}
        ).mappings().fetchone()

        if result:
            nova_qtd = result["quantidade"] + quantidade
            conn.execute(
                text("UPDATE contagens SET quantidade = :quantidade, timestamp = CURRENT_TIMESTAMP WHERE username = :username AND ean = :ean"),
                {"quantidade": nova_qtd, "username": username, "ean": ean}
            )
        else:
            conn.execute(
                text("INSERT INTO contagens (username, ean, quantidade) VALUES (:username, :ean, :quantidade)"),
